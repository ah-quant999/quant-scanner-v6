#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# DO NOT DELETE: 波动率观测（暂未上架实验卡）独立计算脚本
# 用途：拉取主要 A 股指数日 K 收盘，计算「已实现波动率」与「大盘波动率环境 / 企稳信号」，
#       输出 data/experiment/volatility_watch_YYYYMMDD.json，供前端「暂未上架」面板渲染。
# 数据源：baostock 日 K（主，避开 akshare 限流 / mootdx V8 崩溃）→ akshare index_zh_a_hist（备）→ 本地缓存 carry-forward（兜底）。
# 铁律：数据源全失败 / 数据不足 → available=false + 说明，绝不造假（空数据也只给框架+暂无数据）。
#
# 波动率口径（量化标准）：
...
#   指数20日平均收益 ≥ 0 且 波动率5日趋势 < 0  → 企稳回升(stabilize, 绿)：价稳+波动收敛，恐慌释放后的修复
#   指数20日平均收益 < 0 且 波动率5日趋势 < 0  → 阴跌磨底(grind, 琥珀)：价未止跌但杀跌动能衰减
#   指数20日平均收益 ≥ 0 且 波动率5日趋势 ≥ 0  → 反弹分歧(rebound_diverge, 蓝)：价反弹但波动仍在升，假摔/分歧风险
#   指数20日平均收益 < 0 且 波动率5日趋势 ≥ 0  → 恐慌下行(panic, 红)：价跌+波动升，恐慌加速
import os
import sys
import json
import math
...
CACHE_PATH = os.path.join(EXP_DIR, "volatility_cache.json")
...
def regime(idx_metrics):
    """根据各指数 20日收益 与 波动率(短端vs长端) 生成复合信号，并直接回答
    「指数涨 + 波动率下降 = 企稳回升？」这一假设。"""
    ...
    # 短端(5日)相对长端(20日)的高低：<0 表示短端波动率低于长端=波动收敛/下降
    ...
    if code == "stabilize":
        ...
        # 把「企稳回升」所需的两个正向条件一次性列清楚，再说明当前事实为什么不够。
        missing = []
        if not idx_up_20d:
...
def main():
    ...
    out_path = os.path.join(EXP_DIR, f"volatility_watch_{today.strftime('%Y%m%d')}.json")
