...
def get_n_trade_days_ago(n):
    ...
    fallback: 遇周末或BaoStock被拉黑时粗略用 1.4 倍自然日近似"""
    global _bs_blacklisted
    if _bs_blacklisted:
    ...
    try:
        ...
        if lg.error_code == "10001011":
            _bs_blacklisted = True
            print(f"  ⚠️ BaoStock 已被拉黑 ({lg.error_msg}), 交易日历用 1.4 倍自然日近似")
            return (datetime.now() - timedelta(days=int(n * 1.4))).strftime("%Y-%m-%d")
...
def calc_jinzuan_signal(df):
    """金钻趋势信号 — 严格对齐阿狸咪 2026-07-15 今早调试确认版通达信源码(逐行转写)。
    源: tdx_formulas/金钻起涨.tni (最终版, 已确认准确; 不再用旧的等价近似实现)。
    关键: QJ 系列带 IF(H=L,1,...) 一字板/涨跌停保护; CAPITAL≠0(个股) 走 T 分支。
    """
    H = df["high"]
    L = df["low"]
    C = df["close"]
    ...
    # AH:=XMA(XMA(H,25),25);  AL:=XMA(XMA(L,25),25);
    AH = xma(xma(H, 25), 25)
    AL = xma(xma(L, 25), 25)
    # 金钻趋势:=AL-(AH-AL);   金牛:=AH-(AL-AH);
    金钻趋势 = AL - (AH - AL)
    金牛 = AH - (AL - AH)
    ...
    # 黄柱(既有独立信号, 非本公式定义, 保留原判定): 金钻趋势>HIGH
    df["黄柱"] = 金钻趋势 > H
    ...
    if "volume" in df.columns and V.sum() > 0:
        ...
        # T1:=IF(H=L,1,MIN(C,O)-L); T2:=IF(H=L,1,JJ-MIN(C,O));
        # T3:=IF(H=L,1,H-MAX(C,O)); T4:=IF(H=L,1,MAX(C,O)-JJ);
        ...
        # 个股 CAPITAL≠0 → QJn:=QJ0*Tn (IF 的 else 分支)
        QJ1 = QJ0 * T1
        QJ2 = QJ0 * T2
        QJ3 = QJ0 * T3
        QJ4 = QJ0 * T4
        ...
        # DY:=CURRBARSCOUNT=1 AND C<REF(C,1);  scanner 取 iloc[-1] 即末根 → CURRBARSCOUNT=1 恒成立
        DY = (C < ref(C, 1))
        # DY2:=REF(V2,1)-DY;
        DY2 = ref(V2, 1) - pd.Series(DY, index=df.index, dtype=float)
        # 金钻起涨:C>O AND DY2<0.02 AND MA(C,5)>MA(C,60) AND C/REF(C,1)>=1.02 AND H<金牛 AND L<金钻趋势;
...
# ============== 波段是金 副图指标 ==============
def calc_band_signal(df):
    """波段是金 副图指标 -> 波段多头信号(非未来函数)。
    7组EMA递进平均(A1-A7) -> FORCAST(X,6)线性回归预测(VAR1-VAR7) -> 七星转向TOWERC。
    信号规则(基于公式语义, 非自由发挥):
      波段多头 = 主趋势 VAR7 向上(VAR7>REF(VAR7,1)) 且 七星转向蓝柱(TOWERC>=REF(TOWERC,1))
    """
...
# ============== 主力清洗 副图指标 ==============
def calc_zhuli_signal(df):
    """主力清洗 副图指标 -> 主力进场/出货信号(非未来函数)。
    LOW侧 VAR5系(主力进场/洗盘) + HIGH侧 VAR51系(主力拉高/出货)。
    信号规则(基于公式语义, 非自由发挥):
      主力进场 = VAR5>REF(VAR5,1)  (红柱=建仓)
      主力出货 = VAR51>REF(VAR51,1)(青柱=出货, 危险)
      主力洗盘 = VAR5<REF(VAR5,1)  (绿柱=洗盘, 主力压价非坏)
      主力拉高 = VAR51<REF(VAR51,1)(黄柱=拉高)
    """
    O = df["open"]; H = df["high"]; L = df["low"]; C = df["close"]
...
def calc_ema_trends(df):
    """计算7组多周期EMA趋势方向（FORCAST确认）
    
    每组 = (EMA(短) + EMA(中) + EMA(长)) / 3，再做 FORCAST(6) 线性回归
    比较当前FORCAST值与前一周期值判断方向
    
    返回: {'ema_up': up_count, 'ema_dirs': [7个bool], 'ema_score': 0-7}
    """
    try:
        import numpy as np
        if len(df) < 100:
            return {'ema_up': 0, 'ema_dirs': [False]*7, 'ema_score': 0}
        ...
        ohlc = (opens + highs + lows + closes) / 4.0
        ...
        def forcast_val(series, n=6):
            """线性回归预测：拟合最近n个值，返回最后一个回归预测值"""
        ...
        periods = [(3,6,9), (5,10,20), (7,14,28), (9,18,36), (11,22,44), (13,26,52), (21,34,68)]
        directions = []
        
        for s, m, l in periods:
            max_span = max(l + 6, 50)
            seg = ohlc[-max_span:]
            e_s = ema(seg, s)
            e_m = ema(seg, m)
            e_l = ema(seg, l)
            triple = (e_s + e_m + e_l) / 3.0
            
            fc = forcast_val(triple, 6)
            fc_prev = forcast_val(triple[:-1], 6)
            directions.append(fc > fc_prev)
        
        up_count = int(sum(directions))
        dirs = [bool(d) for d in directions]
        return {'ema_up': up_count, 'ema_dirs': dirs, 'ema_score': up_count}
    except Exception:
        return {'ema_up': 0, 'ema_dirs': [False]*7, 'ema_score': 0}
...
# === 东方财富限流自愈: 限流检测 + 冷却期 + 指数退避 ===
# 本机IP在push2实时行情接口上易被东财限流(RemoteDisconnected/超时)。
# 一旦连续失败达阈值, 进入冷却期, 期间所有东财调用直接跳过(自动切回mootdx/baostock主链路),
# 避免反复猛打加重限制; 冷却结束后自动恢复。
_eastmoney_cooldown_until = 0.0      # 冷却截止时间戳(time.time()); 0=未冷却
_eastmoney_consec_fails = 0
_EM_COOLDOWN_SEC = 300               # 限流后冷却5分钟
_EM_FAIL_THRESHOLD = 3               # 连续失败N次进入冷却
_EM_MAX_RETRIES = 3                  # 单次调用最大重试次数(覆盖全局 MAX_RETRIES=1)
def _eastmoney_available():
    """东财是否可调用(冷却期内返回False, 调用方应直接跳过/走主链路)"""
    return time.time() >= _eastmoney_cooldown_until
def _eastmoney_mark_fail():
    global _eastmoney_consec_fails, _eastmoney_cooldown_until
    _eastmoney_consec_fails += 1
    if _eastmoney_consec_fails >= _EM_FAIL_THRESHOLD and time.time() >= _eastmoney_cooldown_until:
        _eastmoney_cooldown_until = time.time() + _EM_COOLDOWN_SEC
        print(f"  [东财] 连续失败{_eastmoney_consec_fails}次, 进入冷却 {_EM_COOLDOWN_SEC}s (暂停调用东财, 主链路mootdx/baostock继续)")
def _eastmoney_mark_ok():
    global _eastmoney_consec_fails
    _eastmoney_consec_fails = 0
def _em_backoff_sleep(attempt):
    """指数退避: 1s, 2s, 4s (封顶8s)"""
...
# mootdx 客户端 - 单例(进程内仅初始化一次V8/py_mini_racer, 必须串行访问)
# 根因1: mootdx依赖py_mini_racer(V8), V8内存分区每进程只能初始化一次;
#        多线程各自建客户端会触发 FATAL: Check failed: !IsConfigurablePoolInitialized() → 进程崩溃
# 根因2: 原48线程共享单例并发调bars(), mootdx非线程安全→内部重连泄漏TrafficStatSocket →
#        累积到~170个时Windows杀掉进程 → scanner超时/崩溃
# 修复: 单例 + 全局RLock串行化所有mootdx调用 + 每N次重置释放socket句柄
import threading as _tdx_threading
_tdx_client = None
_tdx_call_count = 0
_tdx_lock = _tdx_threading.RLock()  # 串行化mootdx访问(防线程不安全+准确计数+允许重置嵌套)
_TDX_RESET_INTERVAL = 50  # 每50次mootdx调用后重置客户端，释放累积的socket句柄

# === ⚠️ A股日K是否走mootdx.bars() — 默认关闭 ===
# mootdx.bars() 走 mootdx/utils/holiday.py 的 py_mini_racer(V8) 交易日解密; 部分机器
# (如HOME阿狸咪)的 py_mini_racer C扩展(_mini_racer.dll)版本不匹配, 首次 MiniRacer() 实例化
# 即触发"C级硬崩溃"(AttributeError 'function mr_eval_context not found'), Python try/except
# 抓不住, 进程直接被 abort() 杀死 → 全扫必崩、无法落盘 scan_result.json。该崩溃是 mootdx 的
# 硬C级bug, 任何Python层补丁(单例化/重试/单线程)均无效。
# 故 A股日K 默认不走 mootdx bars(), 改走 BaoStock(源2)→东方财富(源3) 兜底, 数据质量等价且
# 100%稳定。mootdx 仅保留 board 列表抓取(client.quotes 实时行情, 不触发V8)。
# 仅当某机器 py_mini_racer 正常(如小九LEMONCAT机)时可置 True 恢复 mootdx 高速日K。
# 2026-07-14: LEMONCAT 实测 bars() 可用,且 BaoStock 已被拉黑(10001011),本机切回 mootdx;
# 阿狸咪等其它机器保持 False,避免触发其 V8 崩溃。
USE_MOOTDX_KLINE = os.environ.get("COMPUTERNAME", "").upper() == "LEMONCAT"
...
# 延迟导入 GTimg 数据源，避免本地双机 import 时意外触发
_GTIMG = None
def _gtimg():
    global _GTIMG
    if _GTIMG is None:
        import data_source_gtimg as _GTIMG
    return _GTIMG

def _tdx_reset():
    """重置mootdx客户端: 关闭旧连接→释放socket→gc→重建占位(调用方须持有_tdx_lock)。

    2026-07-14 实测: 托管Python 3.13下V8重建安全; 旧的 _tdx_reset() 仅gc不关client，
    导致 _fetch_board_top_mootdx 的 client.quotes() 打开约104个TrafficStatSocket永不释放，
    累积到~170时被Windows终止进程 → 全扫只到51只就断。
    修复: 实际调用 client.close() 再设为 None 让下次调用重建。"""
    global _tdx_call_count, _tdx_client
    _tdx_call_count = 0
...
def _tdx_count_call():
    """递增mootdx调用计数, 达到阈值时触发重置(调用方须持有_tdx_lock)。"""
...
# === 根治 mootdx/py_mini_racer V8 崩溃: MiniRacer 单例化 ===
# mootdx 的 holiday 交易日解密(mootdx/utils/holiday.py:40)每次调用都 new 一个
# py_mini_racer.MiniRacer(); 而 V8 内存分区每进程只能初始化一次, 第二次初始化
# 直接 FATAL: Check failed: !IsConfigurablePoolInitialized() 杀进程(选股观测台全扫必崩)。
# 其 @file_cache 缓存常因缓存目录不可写而失效, 导致每扫一只A股都新建 MiniRacer → 崩。
# 关键: py_mini_racer 是包, mootdx 用 `from py_mini_racer import py_mini_racer` 拿到的是
#   子模块 py_mini_racer.py_mini_racer.MiniRacer (其 __file__ 为 .../py_mini_racer/py_mini_racer.py),
#   必须把单例补丁打在这个子模块的真实类上, 且必须在 mootdx 懒加载前(本模块导入时)生效。
try:
    _pmp = __import__("py_mini_racer.py_mini_racer", fromlist=["MiniRacer"])
    _MiniRacer_orig = _pmp.MiniRacer
    class _MiniRacerSingleton:
        _inst = None
        def __new__(cls, *a, **k):
            if cls._inst is None:
                cls._inst = _MiniRacer_orig(*a, **k)
            return cls._inst
    _pmp.MiniRacer = _MiniRacerSingleton
except Exception:
    pass


# === 专用mootdx单线程: 根治V8(py_mini_racer)崩溃 ===
# mootdx依赖py_mini_racer(V8)做holiday解密; V8内存分区每进程只能初始化一次,
# 多线程并发调mootdx会各自构造MiniRacer() → FATAL: Check failed: !IsConfigurablePoolInitialized() → 进程崩溃。
# 所有mootdx访问(建客户端/bars/quotes/stocks)统一收口到本单线程, V8仅初始化一次, 且天然串行化消除socket泄漏。
from concurrent.futures import ThreadPoolExecutor as _TdxThreadPoolExecutor
_tdx_executor = _TdxThreadPoolExecutor(max_workers=1, thread_name_prefix="tdx-single")


def _tdx_run(func, *args, **kwargs):
    """在专用mootdx单线程中执行func并取回结果(V8仅初始化一次, 线程安全)"""
    return _tdx_executor.submit(func, *args, **kwargs).result()


def _tdx_fetch_bars(code, bars):
    """在专用mootdx单线程执行bars() (经_tdx_run调用, 勿直接调)"""
    global _tdx_client
    with _tdx_lock:
        ...
        for attempt in range(_EM_MAX_RETRIES):
            try:
                ...
                if r.status_code != 200:
                    _eastmoney_mark_fail()
                    if attempt < _EM_MAX_RETRIES - 1:
                        _em_backoff_sleep(attempt)
                    continue
...
def _get_tdx_client():
    """获取mootdx客户端(单例, 自动选最快服务器, 进程内仅初始化一次V8)"""
    global _tdx_client
...
# BaoStock 单例
_bs_logged_in = False
_bs_blacklisted = False  # 一旦被服务端拉黑(如10001011),本进程不再尝试登录

def _bs_login():
    """BaoStock登录(单次); 若已被拉黑则直接返回False"""
    global _bs_logged_in, _bs_blacklisted
    if _bs_blacklisted:
        return False
    if not _bs_logged_in:
        try:
            ...
            if _bs_logged_in:
                ...
                # 10001011 = 黑名单用户, 服务端已拉黑本机IP, 后续不必再试
                if lg.error_code == "10001011":
                    _bs_blacklisted = True
                    print(f"[数据源] BaoStock 已被服务端拉黑 ({lg.error_msg}), 本进程跳过 BaoStock")
                else:
...
def _fetch_board_top(board_fs, board_label, top_n, fid="f6"):
    """通用: 获取某板块按字段排序的Top N (东方财富兜底, 带限流冷却+指数退避)"""
    if not _eastmoney_available():
        return []
    for attempt in range(_EM_MAX_RETRIES):
        try:
            ...
            if attempt < _EM_MAX_RETRIES - 1:
                _em_backoff_sleep(attempt)
            continue
    print(f"[WARN] 东方财富获取{board_label}列表失败(限流/重试耗尽)")
    return []
...
def fetch_volume_top_stocks(top_cy=None, top_kc=None, top_zb=None, top_hk=None):
    ...
    if CLOUD_RUNNER:
        try:
            return _gtimg().fetch_volume_top_stocks_gtimg(top_cy, top_kc, top_zb, top_hk)
        except Exception as e:
            print(f"  [GTimg] 云端股池获取失败: {e}")
            return []
    ...
    cy = _tdx_run(_fetch_board_top_mootdx, 0, ["300"], "创业板", top_cy)
    if not cy:
    ...
    kc = _tdx_run(_fetch_board_top_mootdx, 1, ["688"], "科创板", top_kc)
    if not kc:
        print("    mootdx失败, 尝试东方财富...")
    ...
    zb_sz = _tdx_run(_fetch_board_top_mootdx, 0, ["000", "001", "002", "003"], "主板", top_zb // 2 + 20)
    zb_sh = _tdx_run(_fetch_board_top_mootdx, 1, ["600", "601", "603", "605"], "主板", top_zb // 2 + 20)
...
def fetch_a_daily(code, bars=None):
    """获取A股日K数据 - 三重源: mootdx → BaoStock → 东方财富；云端优先 GTimg"""
    global _bs_consecutive_fails, _data_source_stats, _tdx_call_count
    if bars is None:
        bars = DAILY_BARS

    # 云端 runner：直接走腾讯 GTimg 前复权日K
    if CLOUD_RUNNER:
        try:
            df = _gtimg().fetch_a_daily_gtimg(code, market="sh" if code.startswith(("6", "688", "9")) else "sz", bars=bars)
    ...
    # 源1: mootdx (通达信直连) — 默认关闭, 见 USE_MOOTDX_KLINE 说明
    # ⚠️ mootdx.bars() 会触发 py_mini_racer(V8) 硬崩溃(部分机器首次实例化即 abort 杀进程),
    #   故默认 False 跳过, 直接走 BaoStock(源2)→东方财富(源3) 兜底。
    if USE_MOOTDX_KLINE:
        try:
            df = _tdx_run(_tdx_fetch_bars, code, bars)
            if df is not None and len(df) >= 60:
                result = pd.DataFrame({
    ...
    # 连续失败过多或被服务端拉黑时跳过，避免拖慢整体扫描
    if (not _bs_blacklisted) and (_bs_consecutive_fails < _BS_FAIL_SKIP_THRESHOLD):
        try:
            _bs_login()
            import baostock as bs
    ...
    # 源3: 东方财富 (兜底, 带限流冷却+指数退避)
    # 东财处于限流冷却期: 直接放弃本次兜底, 避免加重限制(主链路mootdx/baostock已尝试)
    if not _eastmoney_available():
        _data_source_stats["fail"] += 1
        with _tdx_lock:
            _tdx_call_count += 1
    ...
    _data_source_stats["fail"] += 1
    # 定期重置mootdx客户端(释放累积的socket句柄)
    with _tdx_lock:
        _tdx_call_count += 1  # 失败也算一次调用
        if _tdx_call_count >= _TDX_RESET_INTERVAL:
            _tdx_reset()
    return None
...
def fetch_hk_daily(code, bars=None):
    ...
    try:
        import efinance as ef
        # efinance港股代码需要 hk 前缀 + 5位（如 hk00700）
        df = ef.stock.get_quote_history(f"hk{code.zfill(5)}")
        if df is not None and len(df) >= 60:
            result = pd.DataFrame({
...
def check_stock_signals(code, name, market="sh", board_label="", volume_amount=0, turnover_rate=0, mv_yi=0, fund_type=""):
    ...
    try:
        ...
        # 多周期EMA趋势确认（fqz666进阶指标）
        ema_trends = calc_ema_trends(df)
        ...
        return {
            ...
            "pct_chg": round(pct, 2),
            "pct_chg_5d": calc_pct_chg_n(df, 5),
            ...
            "ema_up": ema_trends['ema_up'],
            "ema_dirs": ema_trends['ema_dirs'],
            "ema_score": ema_trends['ema_score'],
...
def update_gold_pool_from_scan(output):
    ...
    for s in pool.get("stocks", {}).values():
        if s.get("signal_count") is None:
            # 优先从 history 最近一天取，否则 fallback 到 max_signal
            history = s.get("history", [])
            if history:
                s["signal_count"] = history[-1].get("signal_count", s.get("max_signal", 0))
            else:
                s["signal_count"] = s.get("max_signal", 0)
            if not isinstance(s["signal_count"], int) or s["signal_count"] < 0:
                s["signal_count"] = 0
    ...
    # 添加/更新本次扫描中有信号的股票
    # 候选宇宙限制放宽：signal_count>=2 的信号股允许入池（无论是否在候选池内），
    # 避免成交额前100候选池与市场实际信号股错配导致金股池枯竭。
    # signal_count<2 的股票仍限制在候选池内，防止低质量小盘股泛滥。
    for s in output.get("all_results", []):
        key = f"{s['market']}_{s['code']}"
        sig = s.get("signal_count", 0)
        if cand_keys is not None and key not in cand_keys and sig < 2:
            continue
        ...
        cand_src = cand.get(key, {}).get("sources", []) if cand else []
        if is_new:
            pool["stocks"][key] = {
                "code": s["code"],
                "name": s["name"],
    ...
    save_gold_pool(pool)
    print(f"  金股池已更新: {pool['total_count']} 只 (信号+研报)")
    print(f"    候选股池: {pool['candidate_total']} 只 | 本批新增研报金股 {research_added} 只 (保留{GOLD_POOL_DAYS}个交易日)")
    return pool
    ...
    for s in watch_output.get("all_results", []):
        ...
        if key in pool["stocks"]:
            ...
            pool["stocks"][key]["history"].append({
                ...
                "close": s["close"],
                "pct_chg": s["pct_chg"],
                "pct_chg_20d": s.get("pct_chg_20d", 0),
            ...
            pool["stocks"][key]["board_label"] = s.get("board_label", pool["stocks"][key].get("board_label", ""))
            pool["stocks"][key]["fund_type"] = s.get("fund_type", pool["stocks"][key].get("fund_type", ""))
        elif s.get("signal_count", 0) >= 2 and (cand_keys is None or key in cand_keys):
            # 盘中新发现双线以上，加入金股池（仅限候选股池内）
            pool["stocks"][key] = {
                "code": s["code"],
                "name": s["name"],
                "market": s["market"],
...
def scan_market(scan_a=True, scan_hk=True, max_stocks=None, resume=False):
    ...
    # 并发从48降至16: 根治socket泄漏后, 每线程独立客户端, 16并发已足够快且峰值socket可控
    MAX_WORKERS = 16
    ...
    try:
        ...
        while pending and time.time() < batch_deadline:
            ...
            for future in done:
                ...
                if result is not None and market != "hk" and result.get("close"):
                    should_verify = result.get("signal_count", 0) >= 2
                    if should_verify:
                        try:
                            verify_df = pd.DataFrame([{"close": result["close"]}])
    ...
    try:
        ...
        # guanlan_extractor 已废弃: import 时会 print("已废弃，跳过") 并 sys.exit(0),
        # 若只捕获 ImportError 会让 SystemExit 杀掉整个扫描进程, 导致 scan_result.json 无法保存。
        # 捕获 SystemExit 使其仅跳过观澜关联, 不影响后续存盘。
        guanlan_data = None
...
def watch_gold_pool():
    ...
    for idx, stock_info in enumerate(stocks_list):
        ...
        # 每30只重置BaoStock/东财失败计数，防一次性永久失活
        if idx > 0 and idx % 30 == 0:
            global _bs_consecutive_fails, _eastmoney_consec_fails, _eastmoney_cooldown_until
            _bs_consecutive_fails = 0
            _eastmoney_consec_fails = 0
