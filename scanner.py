...
# 云端模式 (GitHub Actions): GitHub Runner 在 Ubuntu Linux 上,
# py_mini_racer(V8) 有正常的编译支持, 不会像 Windows 那样崩溃。
# 因此云端直接启用 mootdx bars() 通达信直连, 跳过被美国 IP 封禁的 HTTP 源。
CLOUD_RUNNER = os.environ.get("CLOUD_RUNNER", "").lower() == "true"
if CLOUD_RUNNER:
    _BS_FAIL_SKIP_THRESHOLD = 0  # 云端 baostock 不通, 跳过
    USE_MOOTDX_KLINE = True      # Ubuntu V8 正常, 启用 mootdx 通达信直连
...
def update_gold_pool_from_scan(output):
    ...
    # 添加/更新本次扫描中有信号的股票（仅限候选股池内）
    for s in output.get("all_results", []):
        key = f"{s['market']}_{s['code']}"
        # 候选宇宙限制：不在股池内的信号股不进金股（未构建候选池时放行）
        if cand_keys is not None and key not in cand_keys:
            continue
