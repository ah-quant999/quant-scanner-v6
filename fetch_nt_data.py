...
def generate_calendar():
    ...
    while (y, m) <= (end_date.year, end_date.month):
        ...
        hk_2026 = {
            1:  [1],        # 元旦
            2:  [17,18,19], # 农历新年（年初一至初三）
            4:  [3,6],      # 耶稣受难节+复活节星期一
            5:  [1,25],     # 劳动节+佛诞
            6:  [19],       # 端午节
            7:  [1],        # 香港特区成立纪念日
            9:  [28],       # 中秋节翌日（9/27周日，周一补假）
            10: [1,21],     # 国庆日+重阳节
            12: [25],       # 圣诞节
        }
        # 2027年香港公众假期（预估）
        hk_2027 = {
            1:  [1],        # 元旦
            2:  [8,9,10],   # 农历新年（年初一至初三）
            3:  [26,29],    # 耶稣受难节+复活节星期一
            4:  [5],        # 清明节
            5:  [3,12],     # 劳动节补假(5/1周六)+佛诞
            6:  [8],        # 端午节
            7:  [1],        # 香港特区成立纪念日
            9:  [27],       # 中秋节翌日
            10: [1,11],     # 国庆日+重阳节补假(10/10周日)
            12: [27],       # 圣诞节补假(12/25周六)
        }
        hk_map = {2026: hk_2026, 2027: hk_2027}
        if y in hk_map and m in hk_map[y]:
            for dd in hk_map[y][m]:
                calendar_events.append({
                    'date': f'{y}-{m:02d}-{dd:02d}',
                    'title': '港股休市',
                    'type': 'hk_holiday'
                })

        # ── 美股休市（美国交易所休市日，仅影响美股交易） ──
        us_holidays_2026 = {
            1:  [1],        # 元旦
            2:  [16],       # 总统日（2月第三个周一）
            4:  [3],        # 耶稣受难节
            5:  [25],       # 阵亡将士纪念日（5月最后一个周一）
            7:  [3],        # 独立日（7月4日周六→7月3日周五休市）
            9:  [7],        # 劳动节（9月第一个周一）
            11: [26],       # 感恩节（11月第四个周四）
            12: [25],       # 圣诞节
        }
        us_holidays_2027 = {
            1:  [1,18],     # 元旦+马丁路德金日（1月第三个周一）
            2:  [15],       # 总统日
            3:  [26],       # 耶稣受难节
            5:  [31],       # 阵亡将士纪念日
            7:  [5],        # 独立日（7/4周日→7月5日周一休市）
            9:  [6],        # 劳动节
            11: [25],       # 感恩节
            12: [25,27],    # 圣诞节（12/25周六→12/27周一补假）
        }
        us_map = {2026: us_holidays_2026, 2027: us_holidays_2027}
        if y in us_map and m in us_map[y]:
            for dd in us_map[y][m]:
                calendar_events.append({
                    'date': f'{y}-{m:02d}-{dd:02d}',
                    'title': '美股休市',
                    'type': 'us_holiday'
        ...
        # ── 苹果重要事件 ──
        apple_events = [
            ('2026-06-08', '苹果WWDC·Day1'),
            ('2026-06-09', '苹果WWDC·Day2'),
            ('2026-06-10', '苹果WWDC·Day3'),
            ('2026-09-08', '苹果秋季发布会'),
    ...
    # ── 港股休市（2026-2027 香港公众假期）──
    hk_holidays = [
        ('2026-01-01', '港股休市（元旦）'),
        ('2026-02-17', '港股休市（农历年初一）'),
        ('2026-02-18', '港股休市（农历年初二）'),
        ('2026-02-19', '港股休市（农历年初三）'),
        ('2026-04-04', '港股休市（清明节）'),
        ('2026-04-06', '港股休市（复活节）'),
        ('2026-04-07', '港股休市（复活节）'),
        ('2026-05-01', '港股休市（劳动节）'),
        ('2026-05-25', '港股休市（佛诞）'),
        ('2026-06-19', '港股休市（端午节）'),
        ('2026-07-01', '港股休市（香港回归纪念日）'),
        ('2026-10-01', '港股休市（国庆日）'),
        ('2026-10-12', '港股休市（重阳节）'),
        ('2026-12-25', '港股休市（圣诞节）'),
        ('2026-12-26', '港股休市（节礼日）'),
        ('2027-01-01', '港股休市（元旦）'),
        ('2027-02-06', '港股休市（农历年初一）'),
        ('2027-02-07', '港股休市（农历年初二）'),
        ('2027-02-08', '港股休市（农历年初三）'),
        ('2027-04-05', '港股休市（清明节）'),
        ('2027-05-01', '港股休市（劳动节）'),
        ('2027-05-21', '港股休市（佛诞）'),
        ('2027-06-11', '港股休市（端午节）'),
        ('2027-07-01', '港股休市（香港回归纪念日）'),
        ('2027-10-01', '港股休市（国庆日）'),
        ('2027-10-18', '港股休市（重阳节）'),
        ('2027-12-25', '港股休市（圣诞节）'),
        ('2027-12-26', '港股休市（节礼日）'),
    ]
    for hd, htitle in hk_holidays:
        calendar_events.append({'date': hd, 'title': htitle, 'type': 'hk_holiday'})

    # ── 美股休市（2026-2027 纽交所/纳斯达克休市）──
    us_holidays = [
        ('2026-01-01', '美股休市（元旦）'),
        ('2026-01-19', '美股休市（马丁·路德·金纪念日）'),
        ('2026-02-16', '美股休市（总统日）'),
        ('2026-04-02', '美股休市（耶稣受难日）'),
        ('2026-05-25', '美股休市（阵亡将士纪念日）'),
        ('2026-06-19', '美股休市（六月节）'),
        ('2026-07-03', '美股休市（独立日前一天）'),
        ('2026-09-07', '美股休市（劳动节）'),
        ('2026-11-26', '美股休市（感恩节）'),
        ('2026-12-25', '美股休市（圣诞节）'),
        ('2027-01-01', '美股休市（元旦）'),
        ('2027-01-18', '美股休市（马丁·路德·金纪念日）'),
        ('2027-02-15', '美股休市（总统日）'),
        ('2027-03-26', '美股休市（耶稣受难日）'),
        ('2027-05-31', '美股休市（阵亡将士纪念日）'),
        ('2027-06-18', '美股休市（六月节）'),
        ('2027-07-05', '美股休市（独立日）'),
        ('2027-09-06', '美股休市（劳动节）'),
        ('2027-11-25', '美股休市（感恩节）'),
        ('2027-12-24', '美股休市（圣诞节前夕）'),
    ]
    for ud, utitle in us_holidays:
        calendar_events.append({'date': ud, 'title': utitle, 'type': 'us_holiday'})

    # ── 新加坡公假（2026-2027）──
    sg_holidays = [
        ('2026-01-01', 'SG公假（元旦）'),
        ('2026-02-17', 'SG公假（农历新年）'),
        ('2026-02-18', 'SG公假（农历新年翌日）'),
        ('2026-04-03', 'SG公假（耶稣受难日）'),
        ('2026-05-01', 'SG公假（劳动节）'),
        ('2026-05-20', 'SG公假（卫塞节）'),
        ('2026-05-27', 'SG公假（哈芝节）'),
        ('2026-08-09', 'SG公假（国庆日）'),
        ...
        ('2026-12-25', 'SG公假（圣诞节）'),
        ('2027-01-01', 'SG公假（元旦）'),
        ('2027-02-06', 'SG公假（农历新年）'),
        ('2027-02-07', 'SG公假（农历新年翌日）'),
        ('2027-03-26', 'SG公假（耶稣受难日）'),
        ('2027-05-01', 'SG公假（劳动节）'),
        ('2027-05-09', 'SG公假（卫塞节）'),
        ('2027-08-09', 'SG公假（国庆日）'),
        ('2027-11-08', 'SG公假（屠妖节）'),
    ...
    for sd, stitle in sg_holidays:
        calendar_events.append({'date': sd, 'title': stitle, 'type': 'sg_holiday'})

    # 去重并按日期排序
    # 第一步：精确去重（同title+同date）
    seen = set()
    unique_events = []
    ...
    # 第二步：子串去重（同一日期，若A标题是B标题的子串，保留更详细的B）
    # 典型场景："港股休市" 与 "港股休市（元旦）" 同日期，丢弃前者
    deduped = []
    by_date_sub = {}
    for e in unique_events:
        d = e["date"]
        if d not in by_date_sub:
            by_date_sub[d] = []
        # 检查是否有已存在的事件是当前事件的子串，或当前事件是已存在的子串
        replaced = False
        for idx, existing in enumerate(by_date_sub[d]):
            if existing["title"] in e["title"]:
                # 当前事件更详细，替换旧事件
                by_date_sub[d][idx] = e
                replaced = True
                break
            elif e["title"] in existing["title"]:
                # 旧事件更详细，丢弃当前事件
                replaced = True
                break
        if not replaced:
            by_date_sub[d].append(e)
    
    # 展平
    result = []
    for d in sorted(by_date_sub.keys()):
        result.extend(by_date_sub[d])
