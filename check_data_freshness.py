...
def is_from_last_trading_day(ts_str):
    """检查时间戳是否在最近交易日或之后（用于周末宽限判定）"""
    ts = str(ts_str).strip()
    # 手动解析时间戳，不受格式串长度误导
    for sep in (' ', 'T'):
        if sep in ts:
            date_part, time_part = ts.split(sep, 1)
            time_part = time_part[:5]  # HH:MM，丢弃秒部分
            try:
                y, m, d = map(int, date_part.split('-'))
                h, mi = map(int, time_part.split(':'))
                dt = datetime.datetime(y, m, d, h, mi)
                return dt.date() >= get_last_trading_day()
...
def check_all():
    ...
    today_is_weekend = now.weekday() >= 5  # 5=周六, 6=周日
    if today_is_weekend:
        last_trade = get_last_trading_day()
        print(f"  周末模式: 最近交易日为 {last_trade}")
    print("=" * 55)
    ...
    for filename, (label, icon) in DATA_SOURCES.items():
        ...
        if hours > 48:
            # 周末宽限：数据来自最近交易日 → 算正常
            if today_is_weekend and is_from_last_trading_day(ts):
                ok_count += 1
                print(f"  {icon} {label:12s}  ✅ {days}天前 (周末宽限)")
            else:
            ...
            if today_is_weekend and is_from_last_trading_day(ts):
                ok_count += 1
                print(f"  {icon} {label:12s}  ✅ {int(hours)}小时前 (周末宽限)")
            else:
