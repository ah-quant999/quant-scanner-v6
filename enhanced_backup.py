...
def sync_dual_machine():
    ...
    issues = []
    try:
        # 仅暂存源码改动：先 add -A 再 unstage data/ dist/，
        # 避免把双机各自重生成的派生数据卷进 rebase → 永绝 UU 冲突。
        subprocess.run(['git', 'add', '-A'], cwd=BASE, capture_output=True, text=True, timeout=30)
        subprocess.run(['git', 'reset', '-q', '--', 'data', 'dist'], cwd=BASE, capture_output=True, text=True)
        ...
        # 安全拉取（取代 git pull --rebase：丢弃本地派生数据、干净树 rebase，冲突安全 pop）
        ok = safe_pull()
        if not ok:
            issues.append("git 同步失败")
            print(f"  ⚠ git 同步失败（网络或分叉）")
        else:
            print(f"  ✓ git 同步 origin/main")
