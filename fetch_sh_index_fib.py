...
def fetch_index_history(symbol, days=120):
    """获取指数历史K线（主数据源 baostock，备用 akshare）"""
    history = _fetch_baostock(symbol, days)
    if history and len(history) >= 5:
        # 检查数据新鲜度：最后一条是否是最近2个交易日内的
        last_date = history[-1]['date']
        days_ago = (datetime.now() - datetime.strptime(last_date, "%Y-%m-%d")).days
        if days_ago <= 3:
            return history
        else:
            print(f"  ⚠️ baostock数据滞后({last_date}, 距今{days_ago}天)，尝试akshare补充...")
            ak_history = _fetch_akshare_index(symbol, days)
            if ak_history:
                # 合并：以akshare为主，补充baostock早期数据
                ak_dates = set(d['date'] for d in ak_history)
                extra = [d for d in history if d['date'] not in ak_dates]
                merged = sorted(ak_history + extra, key=lambda x: x['date'])
                print(f"  ✓ 合并后{len(merged)}天（akshare:{len(ak_history)} + baostock补充:{len(extra)}）")
                return merged
            return history
    # baostock 完全失败，走 akshare 兜底
    print(f"  ⚠️ baostock失败或数据不足，切换到akshare...")
    return _fetch_akshare_index(symbol, days)


def _fetch_baostock(symbol, days=120):
    """baostock 主数据源"""
    try:
...
def _fetch_akshare_index(symbol, days=120):
    """akshare 备用数据源（指数日K）"""
    try:
        import akshare as ak
        # 映射 baostock symbol → akshare symbol
        ak_map = {
            'sh.000001': 'sh000001',   # 上证指数
            'sz.399001': 'sz399001',   # 深证成指
        }
        ak_sym = ak_map.get(symbol)
        if not ak_sym:
            print(f"  ⚠️ akshare不支持symbol: {symbol}")
            return []
        ...
        for _, row in df.iterrows():
            try:
                data.append({
                    'date': str(row['date']).replace('-', '')[:10] if len(str(row['date'])) == 10 
                          else str(row['date'])[:10],
                    'open': float(row['open']), 'high': float(row['high']),
                    'low': float(row['low']), 'close': float(row['close']),
                    'volume': float(row.get('volume', 0)), 'preclose': 0,
                    'pctChg': round((row['close'] / row['open'] - 1) * 100, 2) if row['open'] > 0 else 0,
                })
            except (ValueError, KeyError, TypeError):
                continue
        print(f"  ✓ akshare获取{len(data)}天K线")
        return data
    except Exception as e:
        print(f"  ⚠️ akshare也失败: {e}")
        return []
...
def _roll_forward(tracked_wins, reversal_date, today_str):
    """窗口全部走完后自动滚动续命：以最近确认反转日为锚点推算下轮，永不让窗口真空"""
    has_future = any(w["status"] in ("future", "active") for w in tracked_wins)
    if has_future:
        return tracked_wins, []
    ...
    for nw in next_cycle:
        try:
            dl = (datetime.strptime(nw["date"], "%Y-%m-%d") - datetime.strptime(today_str, "%Y-%m-%d")).days
        except Exception:
            dl = 999
        nw["days_left"] = dl
        if dl > 3:
            ...
            nw["status"] = "expired_rolled"
        nw["rolled"] = True

    rolled_wins = list(next_cycle)
    extended = tracked_wins + rolled_wins
...
def build_fib_json(windows, index_data, history):
    ...
    if reversal_date:
        ...
        if next_cycle:
            judgement["key_window"] = f"⚡ 窗口({reversal_date})反转确认，下轮周期已推算"
        else:
            judgement["key_window"] = f"⚡ 窗口({reversal_date})反转确认"
    else:
