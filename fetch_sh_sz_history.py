...
def main():
    ...
    # 补充：K线可能不含当天数据（收盘后延迟），用实时数据追加
    from datetime import date as _date
    today = _date.today()
    today_key = f"{today.month}/{today.day}"
    has_today = any(d.get("date") == today_key for d in amount_history)
    if not has_today:
        sh_amt_rt = sh_rt["amount"] / 1e8   # 元 → 亿元
        sz_amt_rt = sz_rt["amount"] / 1e8
        amount_history.append({
            "date":      today_key,
            "sh_amount": round(sh_amt_rt, 1),
            "sz_amount": round(sz_amt_rt, 1),
            "total":     round(sh_amt_rt + sz_amt_rt, 1),
        })
        print(f"  ✓ 追加今日实时数据: {today_key} 总计{sh_amt_rt + sz_amt_rt:.1f}亿")
