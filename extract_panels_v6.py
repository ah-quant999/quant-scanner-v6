...
window.addEventListener('load',function(){{
  ...
  console.log('[standalone] rendered');
    ...
    idx=f'''<!--
⚠️ 独立页导航 — 禁止删除/覆盖以下页面：
   ✅ 竞彩娱乐 (worldcup.html) — 世界杯/彩票独立展示页，用户明确要求保留
   ✅ 逻辑详解 (guide.html)       — 算法逻辑详解页，用户明确要求保留
   以上两个卡片(_extra列表)严禁移除或重写为空。修改前必须确认用户意图。
-->
...
<div class=ft>不构成投资建议</div></body></html>'''
    ...
    # 同步根目录新鲜共振页到 standalone/，保证独立导航点进去也是最新数据
    _dist_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dist")
    for _p in ('triple_resonance', 'multi_resonance'):
        _src = os.path.join(_dist_dir, f"{_p}.html")
        _dst = os.path.join(OUTPUT_DIR, f"{_p}.html")
        if os.path.exists(_src):
            shutil.copy2(_src, _dst)
            # 根目录版返回链接指向主站(index_master.html)；
            # 拷到 standalone/ 时，index_master.html 降级为 ../index.html（主站）。
            # 注意：href="index.html" 不再降级 —— standalone/ 下 index.html 即独立导航页，
            # 指向它才正确（降级为 ../index.html 反而跳回主站）。
            with open(_dst, 'r', encoding='utf-8') as _f:
                _c = _f.read()
            _c = _c.replace('href="index_master.html"', 'href="../index.html"')
            with open(_dst, 'w', encoding='utf-8') as _f:
                _f.write(_c)
            print(f"  ✓ synced {_p}.html -> {OUTPUT_DIR}/ (返回链接改为 ../index.html)")
        else:
            print(f"  ⚠️ {_p}.html 在 dist/ 不存在，跳过同步（请先运行 update_data_v2.py）")
