...
def push_with_retry(path, role, mode, max_attempts=3):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        # 旧实现用 --autostash：云端 workflow 前置改写 data/*.json(已跟踪, unstaged)，
        #   自动 stash→pull→pop 时因双机数据冲突产生 UU 混乱，卡死后续 git 操作。
        # 现统一走 safe_pull()：拉取前丢弃本地派生数据改动（总会被重新构建，
        #   权威版在 origin/main），干净树 rebase 拉取，冲突安全 pop。永绝后患。
        safe_pull()
        # 2026-07-21 修：必须 safe_pull 之后再写 hb_*.json，否则 safe_pull 的
        # `git checkout -- data/` 会把刚写的新时间戳抹回 origin 旧值，导致
        # 三方协作"九"永远显示陈旧时间（曾经显示 10:14，部署 16:57 后仍 10:14）。
        _, rec = write_heartbeat(role, mode)
        _git("add", "-f", path)
        c = _git("commit", "-m", msg)
...
def main():
    ...
    # 2026-07-21 修：写文件动作前移到 push_with_retry 内部（safe_pull 之后）
    # 避免被 safe_pull 的 `git checkout -- data/` 抹回 origin 旧值
    if args.no_push:
        # --no-push 模式：直接写文件（不会 push，不走 safe_pull）
    ...
    # 默认 push 模式：path 占位（push_with_retry 内会重新写文件）
    path = os.path.join(DATA_DIR, f"hb_{role}.json")
    ok, err = push_with_retry(path, role, args.mode)
    if ok:
        # 重新读一次，确认 push 之后 origin 端的实际内容
        try:
            with open(path, encoding="utf-8") as _f:
                rec = json.load(_f)
            print(f"💓 心跳文件已写: hb_{role}.json (role={role}, mode={args.mode}, time={rec['last_time']})")
        except Exception:
            pass
