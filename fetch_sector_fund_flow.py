...
# 通过 subprocess 调用 `npx -y westock-data-skillhub@1.0.3 board`，
# 由独立拉取器 fetch_sector_fund_flow_westock.py 解析 markdown 表格，
# 提取 mainNetInflow / mainNetInflow5d（万元→亿元归一化为 net / net_5d）。
# 价值：东财限流 / neodata token 失效时，提供另一条完全独立的腾讯兜底源，
#       并为 top_list 回填 5日净额（当东财未给出 net_5d 时）。
# ──────────────────────────────────────────────────────────────
WESTOCK_CACHE = os.path.join(BASE_DIR, "data", "sector_fund_flow_westock.json")
WESTOCK_PULLER = os.path.join(BASE_DIR, "fetch_sector_fund_flow_westock.py")
...
def _carry_forward_history(today):
    """全源失败时：对已有历史的板块沿用上次净额追加今日，
    保证 5/20/60 日累积窗口持续推进（不造假 magnitude、不新增板块）。
    铁律不变：当日 top_list 仍保持空（前端显示"暂无"），此处只维护历史累积。"""
...
def fetch_neodata_5d20d_supplement(sector_names):
    ...
    # 【2026-07-21 修复】移除本地 12h 时间戳过期闸门——它误杀有效 token。
    #   实证：token 长期有效，旧逻辑因 saved_at 超 12h 直接判"不可用"跳过，
    #   导致东财失败时腾讯兜底没兜住。改为 token 字段存在即采用，
    #   真正失效由下方 API 调用失败(HTTP!=200 / suc!=True)优雅降级处理。
    token = None
    for tp in alt_paths:
...
def fetch_from_neodata():
    ...
    # 【2026-07-21 修复】移除本地 12h 时间戳过期闸门（误杀有效 token，见上方说明）。
    #   token 字段存在即采用，真实失效由 API 调用失败兜底。
    for tp in alt_paths:
        if os.path.exists(tp):
            try:
                with open(tp, "r") as f:
...
def fetch_sector_flow():
    ...
    # 【2026-07-13新增】行业数据缺失检测+重试：
    #   方法1(行业)若因 akshare 盘中接口偶发网络错误被吞，会导致最终只有概念数据，
    #   而前端只统计 type='行业' 净额 → 显示 0.0亿（主力板块/板块资金三合一卡）。
    #   此处若发现 top_list 中无任何 type='行业' 条目，则立即重试一次方法1。
    if ak is not None and not any(s.get("type") == "行业" for s in top_list):
        ...
        try:
            ...
            if df is not None and len(df) > 0:
                ...
                print("    ⚠️ 重试方法1仍未获取到行业数据（akshare 接口可能临时不可用）")
        except Exception as e:
            print(f"    ⚠️ 重试方法1失败: {e}")
    ...
    if top_list and ak is not None:
        ...
        # 【腾讯自选股 westock 第三源】对东财/neodata 结果做净额回填：
        # 当某板块 net_5d 为 0（东财未给）但 westock 有 5日值时 → 补 5日累计；
        # 当 net 为 0 但 westock 有当日值时 → 补当日净额。并标记 source。
        ...
        if wsk_items:
            ...
            for item in top_list:
                ...
                if item.get("net", 0) == 0 and w.get("net", 0) != 0:
                    item["net"] = w["net"]
                    bf_net += 1
                # 统一标记来源（去重：net 与 net_5d 同轮回填时也只标一次）
                _ws = item.get("source", "")
                if "腾讯自选股" not in _ws:
        ...
        if wsk_items:
            ...
            if top_list:
                ...
                if out_cnt == 0 and len(top_list) > 3:
                    print("  neodata 仅返回流入板块（缺少流出数据），净流入数字可能虚高！")
                    result["data_note"] = "neodata仅返回流入"
                elif in_cnt == 0:
                    print("  neodata 仅返回流出板块（缺少流入数据），净流出数字可能虚高！")
                    result["data_note"] = "neodata返回流出"
                else:
    ...
    # 【2026-07-24 优化】阈值动态调整：5日≥5天、20日≥10天、60日≥10天；保证 60 日面板不会因累积未满 60 天而全天"暂无"
    ...
    # 生成汇总
    THRESHOLD = 1.0  # 【2026-07-13修复】从5.0降至1.0，避免盘中净额普遍较小时全被过滤导致前端显示"暂无"
    for item in result["top_list"]:
