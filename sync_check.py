...
def git_sync(force=False):
    ...
    if r_status.returncode == 0:
        ...
        if unmerged:
            # 2026-07-18 根因修复：禁止一刀切 reset --hard（会冲掉未推送的修复 commit）。
            # 改为精准还原「origin/main 中存在的已跟踪未合并文件」，保留其他本地提交；
            # 仅未跟踪未合并文件才回退整体 reset --hard（最后手段）。
            tracked, untracked = [], []
            for l in unmerged:
                fn = l[3:].split(' -> ')[-1].strip().replace("\\", "/")
                chk = run(f"git cat-file -e origin/main:{fn}")
                (tracked if chk.returncode == 0 else untracked).append(fn)
            restored = []
            for fn in tracked:
                rc = run(f"git checkout -q origin/main -- {fn}")
                if rc.returncode == 0:
                    restored.append(fn)
                else:
                    untracked.append(fn)  # 还原失败 → 降级
            if restored:
                print(f"  ✓ 已精准还原 {len(restored)} 个未合并文件（保留其他本地提交）")
            if untracked:
                print(f"  ⚠️ 存在未跟踪未合并文件（{len(untracked)} 个），整体 reset --hard origin/main")
            ...
            # 同步远端（安全拉取：先清派生数据、干净树 rebase，绝不遗留冲突）
            ok = safe_pull()
            if not ok:
                print(f"  ❌ git 同步失败（网络或分叉）")
                return False
            print(f"  ✓ 代码已同步")
            return True

    # 安全同步（取代 stash + pull + stash pop：双机数据冲突会导致 pop 冲突、留下 UU 混乱）
    ok = safe_pull()
    if not ok:
        print(f"  ❌ git 同步失败（网络或分叉）")
        if not force:
