...
- **v8 部署后本地残留铁律（2026-08-04 踩坑）**：`deploy_v8.py` 只推线上（tmp 克隆复制），**不提交本地 data/**。每次 update_v8.py 重建后本地必有 51 个 data/*.js 未提交改动，下次 rebase 必被挡。**部署后必须立刻 `git add data/ && git commit && git push`**（云端可能已推同内容 commit，rebase 会自动跳过重复补丁，最终本地=线上干净）。
- **v8 换行符地雷（2026-08-04 已拆）**：`algorithms/scanner.py` + `fetch_inst_trade.py` 历史索引 blob 是 CRLF、`.gitattributes` 要求 `*.py eol=lf` → git status 永远假 M，任何 rebase/pull 被挡（checkout/stash/autocrlf=false 全无效）。已用 `git add --renormalize`+commit（0ce309c）治本。**今后 v8 仓若又出现对称 N± 行假 diff，用 renormalize 而非 checkout/stash**。云端 Actions 对 main 高频 push（08:53 fetch/08:54 build/08:58 deploy 多 workflow 并存），v8 仓本地操作前必先 fetch+rebase。
- **⚠️ cn_fetch 推送崩溃陷阱（2026-08-05 踩坑·f7a42be 修复）**：`api_push_raw.py` 的 `api()` 只捕获 HTTPError，**blob 上传 90s 超时（TimeoutError）未捕获 → 整个推送进程 exit 1，54 个 raw_data 全不落地**（数据抓了但白抓，不触发 build）。修复：timeout 90→300 + except 加 `(TimeoutError, URLError, OSError)` 返回错误 dict（blob 重试/跳过逻辑真正生效）。**workflow 是经 API 从 main 拉脚本的，改脚本必须 push main 后再派发才生效**。cn runner 推送是单点，观察 run 必须盯 step7「📤 经 API 推送」的 conclusion。
- **⚠️ cn_fetch 并发顶替陷阱（2026-08-05 踩坑）**：`v8_cn_fetch.yml` concurrency group=`v8-cn-fetch`（cancel-in-progress:false），**组内已有 run 在跑时，新 dispatch 进入 pending 排队，一旦再有更新 dispatch 到来，排队的旧 run 会被 cancel（组内只保留最新排队者）**。post_close 与 intraday **必须串行派发**（等前一个 completed 再发下一个），否则后派的那个可能被第三方/定时 dispatch 顶掉。派发后 90s 用 API 确认最近两次 run status=completed + conclusion=success（cancelled/failure 都算失败）。
