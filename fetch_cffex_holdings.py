...
def fetch_cffex_data(date_str=None):
    ...
    for attempt in range(max_retry):
        try:
            ...
            log(f"  空数据，重试({attempt+1}/{max_retry})...")
        except Exception as e:
            log(f"  第{attempt+1}次失败: {e}")
        if attempt < max_retry - 1:
            wait = 5 * (2 ** attempt)  # 5/10/20/40s 指数退避
            log(f"  等待 {wait}s 后重试...")
            time.sleep(wait)
...
def main():
    # 2026-07-18 修正：周末/假日同样执行。CFFEX 持仓数据 T+1 发布，
    # 周六需抓取周五（最近交易日）数据。原有非交易日守卫导致周六 T+1
    # 补全跳过，是本日数据停留在 7/16 的根因之一。
    log("=" * 40)
    log("中信期货持仓数据抓取")
    log("=" * 40)
