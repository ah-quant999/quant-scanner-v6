...
| `generate_recommend.py` / `update_multi_resonance_daily.py` | 收盘生成脚本 | close_p2 Group4 必需，曾因未跟踪从main丢失+误判失败(exit 1)；已从backup恢复 | 每日运行 |
...
| `update_worldcup_standalone.py` | 世界杯独立页部署 | worldcup.html 部署必需 | 每日更新 |
| `build_standalone_worldcup.py` | 世界杯独立页生成 | 由 update_worldcup_standalone 调用的生成器 | 每日更新 |
...
| `fetch_worldcup.py` | 世界杯数据抓取+Monte Carlo模拟 | batch_update 调用的世界杯管线核心，缺失则世界杯页不更新；曾因未加白名单被 gitignore 游离 | 每日更新 |
...
**最后核对**: 2026-07-12 12:30 ✅ (修复 pre-commit 钩子读取 .md + 补标漏标核心文件)  
**下次审查**: 2026-07-19 (每周审查)
