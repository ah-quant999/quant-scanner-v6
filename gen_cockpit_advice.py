...
SRC = os.path.join(BASE, "data", "backtest_tdx.json")
OUT = os.path.join(BASE, "data", "cockpit_advice.json")
...
def main():
    ...
    advice = {
        ...
        "watch": watch,
        "avoid": avoid,
        "risk": f"基于金股池历史回测胜率纪律：轻仓试多、单只≤30%、持有{best_pd}日、破买入价-5%止损；共振≥80样本不足，不作为单一信号。",
    }
