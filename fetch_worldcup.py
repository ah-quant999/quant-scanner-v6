...
def build_knockout_schedule(standings, results):
    ...
    if api_data:
        ...
        # ===== 手动覆盖：用户确认的实时比分（API 尚未更新时强制填入） =====
        # 键 (home, away) 为中文队名；标 finished=True + score。
        MANUAL_RESULTS = [
            {'home': '阿根廷', 'away': '瑞士', 'score': '3-1', 'finished': True},
        ]
        for m in api_data:
            for ov in MANUAL_RESULTS:
                if m.get('home') == ov['home'] and m.get('away') == ov['away']:
                    m['score'] = ov['score']
                    m['finished'] = ov['finished']
                    log(f"✍️ 手动覆盖 {ov['home']} vs {ov['away']} → {ov['score']}")
...
def simulate_championship_NEW(knockout, all_teams, standings):
    """Monte Carlo 模拟：基于球队实力的夺冠概率 (修复版 v2)

    策略：
    1. 有真实对阵数据的轮次：严格按对阵图推进，已完赛用真实结果
    2. 缺少对阵数据的轮次（如8强API未更新）：退化回随机配对模式
    3. 已淘汰的球队不可能夺冠

    ⚠️ 2026-07-08修复：原版只读取32强真实结果，16强及之后全靠随机模拟，
       导致已被淘汰的球队（如葡萄牙0-1西班牙）仍有夺冠概率。
    """
    import random, re
    ITER = 5000
    champ_count = {}
    ...
    def resolve_real_result(match):
        score = (match.get('score') or '').strip()
        home, away = match['home'], match['away']
        if not score or score.startswith('TBD') or home == '待定' or away == '待定':
            return None
        parts = score.split()[0]
        if '-' not in parts:
            return None
    ...
    # 按轮次分组（1/4决赛 = 8强，避免重复轮次导致中间退化回随机抽签）
    ROUND_ORDER = ['32强', '16强', '1/4决赛', '半决赛', '三四名决赛', '决赛']
    rounds_data = {}
    for m in knockout:
        rd = m.get('round', '')
        rounds_data.setdefault(rd, []).append(m)

    for _ in range(ITER):
        alive = None

        for rd in ROUND_ORDER:
            matches_this_round = rounds_data.get(rd, [])
            ...
            if not matches_this_round:
                if alive is not None and len(alive) > 1:
                    ...
                    alive = list(next_round)
                continue

            # 情况B：首轮（32强）
            if alive is None:
                winners = []
                for m in matches_this_round:
                    if m.get('home') == '待定' or m.get('away') == '待定':
                        continue
                    rw = resolve_real_result(m)
                    if rw:
                        winners.append(rw)
                    else:
                        winners.append(sim_winner(m['home'], m['away']))
                alive = list(winners)
                continue
            ...
            winners = []
            for m in matches_this_round:
                hn, an = std_name(m['home']), std_name(m['away'])
                # 跳过待定对阵
                if hn == '待定' or an == '待定':
                    continue
                ha = hn in alive
                ...
                if ha and aa:
                    ...
                    if rw:
                        ...
                        winners.append(sim_winner(hn, an))
                elif ha:
                    winners.append(hn)
                elif aa:
                    winners.append(an)
                # else: 双方都不在存活中（数据异常），跳过

            if winners:
                alive = list(winners)
            # 如果winners为空但alive非空（该轮全部待定），保持alive不变进入下一轮

        # 所有轮次结束后，若仍有多队存活（决赛对阵尚未公布/为空），
        # 退化模拟最终决赛，避免直接取 alive[0] 导致冠军被固定
        while alive and len(alive) > 1:
            next_round = []
            random.shuffle(alive)
            for i in range(0, len(alive) - 1, 2):
                next_round.append(sim_winner(alive[i], alive[i + 1]))
...
def main():
    ...
    odds = simulate_championship_NEW(knockout, all_teams, standings)
    if len(odds) >= 3:
        log(f'    TOP3: {odds[0]["n"]} {odds[0]["prob"]}% / {odds[1]["n"]} {odds[1]["prob"]}% / {odds[2]["n"]} {odds[2]["prob"]}%')
    elif odds:
        parts = [f"{o['n']} {o['prob']}%" for o in odds]
        log(f'    TOP: {" / ".join(parts)}')
    else:
        log('    TOP: (空)')
