...
# -*- coding: utf-8 -*-
"""
打新价值评分数据获取 —— 支持申购期/上市首日/上市后追踪 + 可转债
...
def main():
    # 非交易日（周末+A股法定假日）跳过，避免休市日空跑抓空数据
    # 与 run_intraday_scan.py / fetch_industry_map.py 保持一致；fail-safe：模块缺失照跑
    try:
        from is_trading_day import is_trading_day
        if not is_trading_day(datetime.now().date()):
            print("\n⏭️ 今日为非交易日（周末或A股法定假日），跳过打新数据抓取。")
            return 0
    except Exception:
        pass
    ...
    # 家用机(阿狸咪)IP被东方财富封禁，push2行情接口必超时，本脚本应由单位机(小九)运行
    role = ""
    try:
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), ".machine_role"), "r", encoding="utf-8") as f:
            role = f.read().strip()
    except Exception:
        pass
    if role == "ALIMI":
        print("\n⏭️ 本机为家用机(ALIMI)，IP被东方财富封禁，跳过打新数据抓取（避免120s超时）。")
        print("   数据应由单位机(小九, 早1分钟运行)提供。")
        return 0
    ...
    # 合并 applying + pre_listing 一起补充行情
    ...
    # 合并所有结果：新股（申购→待上市→今日上市→追踪）+ 可转债（申购→待上市→今日上市→追踪）
    ...
    print(f"\n✅ 写入 {out_path}")
    stock_cnt = len(applying_results)+len(pre_listing_results)+len(listed_results)+len(tracking_results)
    bond_cnt = len(bond_applying_r)+len(bond_pre_listing_r)+len(bond_listed_results)+len(bond_tracking_results)
    print(f"   新股: {stock_cnt} 只，可转债: {bond_cnt} 只")
    for r in applying_results:
        print(f"   {'⭐' * (r['score']//20)}{r['score']:3d}分 {r['name']}({r['code']}) — {r['recommend']} | {r['board']}")
    for r in pre_listing_results:
        end = f"预计{r['listing_date']}上市" if r.get('listing_date') else r['board']
        print(f"   📋 {r['name']}({r['code']}) — {r['recommend']} | {end}")
    ...
    for r in bond_applying_r:
        tp = r.get('transfer_price', 0)
        sc = r.get('bond_scale', 0)
        print(f"   💰 {r['name']}({r['code']}) — {r['recommend']} | 转股价¥{tp:.2f} 规模{sc:.1f}亿")
    for r in bond_listed_results:
        print(f"   💰📈 {r['name']}({r['code']}) — 首日涨{r['total_return']:.1f}%")
    print(f"\n💡 {summary}")
