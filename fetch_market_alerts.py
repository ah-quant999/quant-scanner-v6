...
def fetch_us_indices():
    """获取美股三大指数（隔夜收盘，新浪gb_接口）
    
    返回: [{'name':'道琼斯','price':52925.15,'pct':-0.25}, ...]
          或 None（失败时）
    """
    try:
        US_SINA_MAP = {
        ...
        if r.status_code != 200:
            log(f"  ✗ 美股指数获取失败 HTTP{r.status_code}")
            return None
        ...
        for line in r.text.strip().split(';'):
            ...
            parts = data_str.split(',')
            if len(parts) >= 4:
                name = US_SINA_MAP.get(code, parts[0])
                price = float(parts[1]) if parts[1] else 0
                # 新浪gb_接口: parts[2]=涨跌幅%(字符串), parts[3]=时间戳
                try:
                    pct = float(parts[2])
        ...
        if result:
            for item in result:
                pct_str = f"{item['pct']:+.2f}%" if item['pct'] is not None else '--'
                log(f"    ✓ {item['name']} {item['price']:,.2f} ({pct_str})")
...
def main():
    ...
    if not indices and not top_sectors and not mood:
        ...
        output = {'update_time': now_str, 'data_available': False, 'summary': '', 'indices': [], 'us_indices': [], 'top_sectors': [], 'bottom_sectors': [], 'mood': {}}
    ...
    if us_indices:
        parts = [f"{u['name']} {u['price']:,.2f} ({u['pct']:+.2f}%)" for u in us_indices]
        print(f"  美股：{', '.join(parts)}")
